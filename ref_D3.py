...
class inpute_values:
    ...
    def import_data(self):
        ...
        self.raw_data = pd.read_csv(os.path.join("Datasets", csv_files[3]))
        raw_data = pd.read_csv(os.path.join("Datasets", csv_files[3]))
    ...
    def model(self, model):
        if model == 0:
            return load_model("model_D3_01.h5")
        elif model == 1:
            return load_model("model_D3_02.h5")
        elif model == 2:
            return load_model("model_D3_03.h5")
...
for model_number in range(0, 3):  # Modelle 1-3 durchlaufen
    ...
    output_prefix = f"prediction_ref_mean_D3_{model_name}_"
...
for model_number in range(0, 3):  # Modelle 1-3 durchlaufen
    model_name = f"Model_{model_number + 1}"
    output_prefix = f"prediction_ref_MICE_D3_{model_name}_"
...
for model_number in range(0, 3):  # Modelle 1-3 durchlaufen
    model_name = f"Model_{model_number + 1}"
    output_prefix = f"prediction_ref_kNN_D3_{model_name}_"
...
for model_number in range(0, 3):  # Modelle 1-3 durchlaufen
    model_name = f"Model_{model_number + 1}"
    output_prefix = f"prediction_ref_RF_D3_{model_name}_"
